...
export default function Pautan({ displayType }) {
            ...
            try {
                while (currentPage <= lastPage) {
                    const response = await fetch(
                        `/api/crud/external_links?page=${currentPage}`,
                        {
                            method: "GET",
                ...
                const deptLinks = sortedLinks.filter((link) =>
                    link.label.toLowerCase().includes("dept")
                );
                const nonDeptLinks = sortedLinks.filter(
                    (link) => !link.label.toLowerCase().includes("dept")
                );
    ...
    const linksToDisplay = () => {
        ...
        if (displayType === "nonDepartment") {
            return showAllNonDept
                ? nonDepartmentLinks
                : nonDepartmentLinks.slice(0, 10);
        }
        return [];
    };
